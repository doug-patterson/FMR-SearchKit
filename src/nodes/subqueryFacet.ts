...
export const filter = ({ field, idPath, subqueryValues }: SubqueryFacetFilter): MongoAggregation =>
  ...
  (
    ...
    ...(optionSearch
      ? [
          {
            $match: {
              [include ? `value.${_.first(include)}` : '_id']: {
                $regex: optionSearch,
                $options: 'i'
              }
            }
      ...
      : []),
    {
      $project: {
        ...
        checked: {
          $in: [
            '$_id',
            _.size(values) && optionsAreMongoIds
