import { DayOfWeekSummaryBarsChart, MongoAggregation } from '../types'
...
export const results = ({
  ...
  agg,
  offset = 0
}: DayOfWeekSummaryBarsChart): MongoAggregation => [
  {
    $group: {
      _id: {
        day: { $dayOfWeek: { date: `$${x}`, timezone: timezoneOffset(offset) } },
        [`${group || 'results'}`]: group
          ? `$${group}${idPath ? '.' : ''}${idPath || ''}`
          : 'results'
      },
